...
public class CaveTrollEntity extends Monster implements GeoEntity {
    ...
    public void tick() {
        ...
        // Lancer de rocher à distance (moins fréquent)
        if (!this.level().isClientSide && this.getTarget() != null && rockThrowCooldown == 0) {
            ...
            if (distance > 64.0D && distance < 400.0D) {
                throwRock();
                rockThrowCooldown = 160; // Plus long que Hill Troll
            }
        }
    }
    ...
    public boolean doHurtTarget(net.minecraft.world.entity.Entity target) {
        ...
        if (flag && target instanceof LivingEntity living) {
            if (slamCooldown == 0 && this.getRandom().nextFloat() < 0.35F) {
                performSlam(living);
                slamCooldown = 120; // Plus long que Hill Troll
            }
        }
    ...
    private void performSlam(LivingEntity target) {
        double knockbackStrength = 3.0D; // Plus puissant
        ...
        if (this.level() instanceof ServerLevel serverLevel) {
            ...
            AABB area = this.getBoundingBox().inflate(5.0D); // Plus grande zone
            ...
            for (LivingEntity entity : nearbyEntities) {
                entity.hurt(this.damageSources().mobAttack(this), 6.0F); // Plus de dégâts
                double dx = entity.getX() - this.getX();
                double dz = entity.getZ() - this.getZ();
                entity.knockback(2.0D, dx, dz);
            }
        }
    }
    ...
    private void throwRock() {
        ...
        rock.shoot(dx, dy + distance * 0.12D, dz, 1.4F, 6.0F);
    ...
    public float getScale() {
        return 1.45F; // Plus grand que Hill Troll
    }
