...
/**
 * Warg - Loup massif, attaque en meute.
 */
public class WargEntity extends Monster implements GeoEntity {
    ...
    private PlayState predicate(AnimationState<WargEntity> event) {
        ...
        if (event.isMoving()) {
            if (this.isSprinting()) {
                event.getController().setAnimation(
                    RawAnimation.begin().then("sprint_pack", Animation.LoopType.LOOP)
                );
            } else {
                event.getController().setAnimation(
                    RawAnimation.begin().then("run_hunt", Animation.LoopType.LOOP)
                );
            }
        } else {
            event.getController().setAnimation(
                RawAnimation.begin().then("idle_prowl", Animation.LoopType.LOOP)
            );
        }
