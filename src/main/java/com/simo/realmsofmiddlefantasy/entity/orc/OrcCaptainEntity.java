...
/**
 * Orc Captain - Field leader of the Orc bands.
 * Commands nearby orcs and rallies them in battle.
 * More disciplined and dangerous than common orcs.
 */
public class OrcCaptainEntity extends Monster implements GeoEntity {
    ...
    private void performRally() {
        ...
        for (Monster orc : nearbyOrcs) {
            orc.heal(4.0F);
            // Buff nearby orcs: Increase attack damage and defense for a short time
            orc.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(orc.getAttribute(Attributes.ATTACK_DAMAGE).getBaseValue() + 1.0D);
            orc.getAttribute(Attributes.ARMOR).setBaseValue(orc.getAttribute(Attributes.ARMOR).getBaseValue() + 2.0D);
