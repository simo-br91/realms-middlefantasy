...
/**
 * Dúnedain Ranger - Elite human ranger from the North.
 * Skilled tracker and warrior, protects the borders of civilized lands.
 */
public class DunedainRangerEntity extends Monster implements GeoEntity, RangedAttackMob {
    
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
    private int combatModeSwitchCooldown = 0;

    public DunedainRangerEntity(EntityType<? extends Monster> type, Level level) {
        super(type, level);
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(1, new RangedBowAttackGoal<>(this, 1.0D, 20, 30.0F));  // Ranged combat
        this.goalSelector.addGoal(2, new MeleeAttackGoal(this, 1.1D, false));  // Melee combat
        this.goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.1D, 1.3D));  // Avoiding stronger monsters
        this.goalSelector.addGoal(7, new RandomStrollGoal(this, 0.7D));  // Random movement
        this.goalSelector.addGoal(8, new LookAtPlayerGoal(this, Player.class, 10.0F));  // Look at player
    ...
    public void tick() {
        super.tick();
        if (combatModeSwitchCooldown > 0) combatModeSwitchCooldown--;
    }
    ...
    public void performRangedAttack(LivingEntity target, float velocity) {
        ...
        // Dúnedain have excellent accuracy
        double d0 = target.getX() - this.getX();
        double d1 = target.getY(0.3333333333333333D) - arrow.getY();
        double d2 = target.getZ() - this.getZ();
        double d3 = Math.sqrt(d0 * d0 + d2 * d2);
        
        arrow.shoot(d0, d1 + d3 * 0.16D, d2, 1.8F, (float)(10 - this.level().getDifficulty().getId() * 4));
    ...
    private PlayState predicate(AnimationState<DunedainRangerEntity> event) {
        if (this.isDeadOrDying()) {
            event.getController().setAnimation(
                RawAnimation.begin().then("animation.dunedain_ranger.hurt", Animation.LoopType.PLAY_ONCE)
            );
        ...
        if (this.swinging) {
            event.getController().setAnimation(
                RawAnimation.begin().then("animation.dunedain_ranger.attack", Animation.LoopType.PLAY_ONCE)
            );
            return PlayState.CONTINUE;
        }
        ...
        if (event.isMoving()) {
            event.getController().setAnimation(
                RawAnimation.begin().then("animation.dunedain_ranger.walk", Animation.LoopType.LOOP)
            );
        } else {
            event.getController().setAnimation(
                RawAnimation.begin().then("animation.dunedain_ranger.idle", Animation.LoopType.LOOP)
            );
        }
