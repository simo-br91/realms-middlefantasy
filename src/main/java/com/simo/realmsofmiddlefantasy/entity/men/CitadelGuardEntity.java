...
public class CitadelGuardEntity extends Monster implements GeoEntity {
    ...
    protected void registerGoals() {
        ...
        // Defensive logic: if HP below threshold, prioritize defense or flee
        this.goalSelector.addGoal(10, new DefensiveFleeGoal(this));
    ...
    /**
     * Defensive and fleeing logic for Citadel Guard.
     * Will flee if health drops below a threshold.
     */
    public static class DefensiveFleeGoal extends Goal {
        private final CitadelGuardEntity guard;
        private final double fleeSpeed;
        private final double fleeDistance;

        public DefensiveFleeGoal(CitadelGuardEntity guard) {
            this.guard = guard;
            this.fleeSpeed = 1.5D;
            this.fleeDistance = 12.0D;
        }

        @Override
        public boolean canUse() {
            return guard.getHealth() < guard.getMaxHealth() * 0.3; // Flee if health is under 30%
        }
        ...
        public void start() {
            if (guard.getTarget() != null) {
                ...
                guard.getNavigation().moveTo(guard.getTarget().getX(), guard.getTarget().getY(), guard.getTarget().getZ(), fleeSpeed);
            }
        }
